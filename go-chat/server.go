...
func handleConnections(w http.ResponseWriter, r *http.Request) {
    ...
    if redisClient.Exists("chat_messages").Val() != 0 {
...
func sendPreviousMessages(ws *websocket.Conn) {
    chatMessages, err := redisClient.LRange("chat_messages", 0, -1).Result()
    if err != nil {
        panic(err)
    }
...
func storeInRedis(msg ChatMessage) {
    ...
    if err := redisClient.RPush("chat_messages", json).Err(); err != nil {
        panic(err)
    }
}
